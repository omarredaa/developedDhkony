...
export default function Navbar() {
  const { cartCount, menuOpen, setMenuOpen, cartOpen, setCartOpen } = useCart();
          ...
          <div className="flex items-center justify-between h-16 mt-3.5 font-bold">
            ...
            <div className="hidden md:flex justify-between items-center gap-3">
              ...
              {user?.role === "Admin" && (
                <Link
                  href="/dashboard"
                  className="flex items-center gap-2  transition text-nowrap"
                >
                  لوحة التحكم
            ...
            <div className="md:hidden pb-4 space-y-4 bg-[#d3c5bb] rounded-3xl mt-4 p-5">
              ...
              {user?.role === "Admin" ? (
                <>
                  ...
                  {isOpen && (
                    <div className="absolute w-40 bg-white shadow-lg rounded-lg border">
                      {user && (
                        <button
                          onClick={logout}
                          className="w-full text-left px-4 py-2  cursor-pointer"
