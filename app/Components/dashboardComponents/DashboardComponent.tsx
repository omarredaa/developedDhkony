...
export default function DashboardComponent() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen w-full bg-gradient-to-br from-black via-zinc-900 to-black text-white">
        {" "}
        <div className="flex flex-col items-center">
          {" "}
  ...
  return (
    <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-hidden mt-10">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-10">
        <h1 className="text-2xl sm:text-3xl font-bold">Dashboard Overview</h1>

        <div className="bg-indigo-600 px-4 py-2 rounded-xl shadow-lg text-sm sm:text-base w-fit">
          {(user && `Welcome, ${user.email}`) || "Welcome!"}
        </div>
      </div>
      {/* Stats Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-white/10 p-5 sm:p-6 rounded-2xl backdrop-blur-xl border border-white/10 shadow-xl hover:scale-105 transition"
          >
            <stat.icon className="mb-4 text-indigo-400" size={28} />
            <p className="text-gray-400 text-sm">{stat.title}</p>
            <h2 className="text-xl sm:text-2xl font-bold mt-2">{stat.value}</h2>
          </div>
        ))}
      </div>
      {/* Top Products */}
      <div className="bg-white/5 rounded-2xl p-4 sm:p-6 border border-white/10 shadow-lg">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[500px] text-left">
            <thead>
              <tr className="text-gray-400 border-b border-white/10 text-sm sm:text-base">
                <th className="pb-3">Product</th>
                <th className="pb-3">Price</th>
                <th className="pb-3">Sold</th>
                <th className="pb-3">Clicks</th>
            ...
            <tbody>
              {products.map((product) => (
                <tr
                  key={product.id}
                  className="border-b border-white/5 hover:bg-white/5 transition text-sm sm:text-base"
                >
                  <td className="py-4">{product.name}</td>
