...
export default function LoginPage() {
  ...
  useEffect(() => {
    sessionStorage.setItem("bootEmailFromSession", email);
  }, [email]);
  const sendOtp = async () => {
    // if (!email) {
    //   alert("أدخل البريد الإلكتروني");
    ...
    try {
      ...
      // Send email as plain string (matches your backend)
  ...
  const verifyOtp = async () => {
    ...
    if (enteredOtp.length !== 6) {
      alert("أدخل رمز مكون من 6 أرقام");
      return;
    }
    ...
    try {
      ...
      // Pass email and otp as query params (works with your current backend)
      const url = new URL(
        `${process.env.NEXT_PUBLIC_API_URL}/api/Auth/verify-otp`,
      );
      url.searchParams.append("email", email);
      url.searchParams.append("otp", enteredOtp);
  ...
  return (
    <div
      dir="ltr"
      className="min-h-screen flex flex-col items-center justify-center px-4 gap-10 bg-gray-100"
    >
      {/* Header */}
      <div className="flex items-center justify-center w-full border-b py-4">
        <Link href="/">
          <img src="/logo.png" alt="logo" className="w-16 cursor-pointer" />
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm w-full max-w-md p-6 sm:p-8">
        {/* Continue as guest */}
        <Link href="/login">
          <button className="w-full border border-gray-300 rounded-lg py-3 text-gray-700 hover:bg-gray-50 transition cursor-pointer mb-4">
            المتابعة كزائر
          </button>
        </Link>
        ...
        {!otpSent && (
          <>
            ...
            <button
              onClick={sendOtp}
              disabled={loading}
              className="w-full bg-[#2b201f] text-white py-3 rounded-lg font-semibold hover:bg-black transition disabled:opacity-50"
            >
